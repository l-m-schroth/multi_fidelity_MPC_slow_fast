...
def eom_2d_quadro_springpend_explicit(x, u, params):
    ...
    x_dot[9] = (1.0*M*f_ext_my*ca.cos(x[4]) + 1.0*M*f_ext_mz*ca.sin(x[4]) - 2.0*M*m*x[8]*x[9] + 1.0*c*m*x[10]*ca.sin(x[2] - x[4]) + 1.0*c*m*x[11]*ca.sin(x[2] - x[4]) - 1.0*f_ext_dy*m*ca.cos(x[4]) - 1.0*f_ext_dz*m*ca.sin(x[4]))/(M*m*x[3])
    x_dot[10] = u[0]
    x_dot[11] = u[1]
    return x_dot
